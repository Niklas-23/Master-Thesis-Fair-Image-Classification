...
import src.fairness_libraries.blackbox_multiclass_fairness.MulticlassBalancer as balancers
...
class CLFRates:
    def __init__(self,
        ...
        self.tnr = 0 if self.tnr is None or (isinstance(self.tnr, float) and math.isnan(self.tnr)) else self.tnr
        self.tpr = 0 if self.tpr is None or (isinstance(self.tpr, float) and math.isnan(self.tpr)) else self.tpr
        self.fnr = 0 if self.fnr is None or (isinstance(self.fnr, float) and math.isnan(self.fnr)) else self.fnr
        self.fpr = 0 if self.fpr is None or (isinstance(self.fpr, float) and math.isnan(self.fpr)) else self.fpr
