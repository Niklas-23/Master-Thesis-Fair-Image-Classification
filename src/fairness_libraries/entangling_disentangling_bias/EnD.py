...
class EnDLoss(torch.nn.Module):
    def __init__(self, model, alpha=0.1, beta=0.1):
        super(EnDLoss, self).__init__()
        self.model = model
        self.alpha = alpha
        self.beta = beta
        self.hook = Hook(model.avgpool, backward=False)

    def forward(self, output, target_labels, protected_features):
        loss = ce(output, target_labels)
        abs = abs_regu(self.hook, target_labels, protected_features, self.alpha, self.beta)
        return loss, abs
