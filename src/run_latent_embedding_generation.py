...
def main(args):
    ...
    if dataset_name == "celeba":
        ...
        experiment_dataset = LFWDataset(target_name="Big Nose")
    ...
    mitigation_approaches.append(
        get_fair_feature_distillation_benchmark(experiment_dataset, device, batch_size, model_path, baseline_model_path))
    mitigation_approaches.append(
        get_orthogonal_representations_benchmark(experiment_dataset, device, batch_size, model_path))
    ...
    for approach in mitigation_approaches:
        name, embeddings = approach.generate_latent_embedding(model_state_version="best_f1_state")
        os.makedirs(f"{current_file_path}/../latent_embeddings/{model_path}", exist_ok=True)
        np.save(file=f"{current_file_path}/../latent_embeddings/{model_path}/{name} Embedding.npy", arr=embeddings)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--dataset_name', type=str, help='Name of the dataset',
                        choices=['lfw', 'celeba', 'utk', 'fair_face'])
